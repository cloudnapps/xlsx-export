...
var getHeaderColumns = function (headers) {
    var cols = "";
...
var getHeaderRow = function (headers, sharedStrings, convertedSharedStrings) {
	var row = '<x:row r="1" spans="1:'+ headers.length + '">';
	var type = 'string';
	headers.forEach(function (header, index) {
		var cellRef = helpers.getColumnRef(index + 1, 0);
		if (!helpers.typeHandlers[type]) {
			type = 'string';
		}

		row += helpers.typeHandlers[type](cellRef, header.caption, 0, sharedStrings, convertedSharedStrings);
	});
	row += '</x:row>';
	return row;
};
...
function XMLStream(options) {
	...
	this.push(getHeaderRow(options.headers, this.sharedStrings, this.convertedSharedStrings));
}
...
XMLStream.prototype._transform = function (doc, encoding, callback) {
	...
	Object.keys(this.map).forEach(function (mapKey, keyIndex) {
		...
		row += helpers.typeHandlers[type](cellRef, doc[mapKey], null, self.sharedStrings, self.convertedSharedStrings);
	});
