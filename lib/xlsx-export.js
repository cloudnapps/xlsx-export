...
var SheetStream = require('./sheetStream');
...
function getTemplateFiles () {
	var zip = new require('node-zip')(templateXLSX, {base64: true, checkCRC32: false});
	return zip.files;
}

function getQueue(files, archive) {
	var queue = [];
	Object.keys(files).forEach(function (file) {
		var data = files[file]._data;
		var content;
		if (data) {
			content =  new Buffer(files[file]._data.getContent(), 'binary');
		}
		else {
			content = "";
		}
		if (files[file].name !== 'xl/worksheets/sheet.xml' && files[file].name !== 'xl/sharedStrings.xml') {
			queue.push(function (callback) {
				archive.entry(content, {name: files[file].name}, callback);
			});
		}
...
function XlsxPacker(sheetStream) {
	var archive = new Packer();
	var files = getTemplateFiles();
	var queue = getQueue(files, archive);

	queue.push(function (callback) {
		archive.entry(sheetStream, {name: 'xl/worksheets/sheet.xml'}, callback);
	});

	queue.push(function (callback) {
		sharedStringsFront = sharedStringsFront.replace(/\$count/g, sheetStream.sharedStrings.length);
		archive.entry(sharedStringsFront + sheetStream.convertedSharedStrings.join('') +  sharedStringsBack, {name: 'xl/sharedStrings.xml'}, callback);
	});
...
module.exports = function (options) {
	var xlsxStream = new SheetStream(options);
	options.stream.pipe(xlsxStream);
	return new XlsxPacker(xlsxStream);
};